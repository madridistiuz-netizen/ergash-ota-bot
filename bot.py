...
def mrt3t_groups_keyboard(lang):
    ...
    data = load_data()
    buttons = []
    for group in data["mrt_3t_groups"].keys():
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    greet = {
        "ru": "👋 Добро пожаловать в клинику *Эргаш-Ота*!\n\nВыберите язык:",
        "uz": "👋 *Эргаш-Ота* klinikasiga xush kelibsiz!\n\nTilni tanlang:",
        "kz": "👋 *Эргаш-Ота* клиникасына қош келдіңіз!\n\nТілді таңдаңыз:",
    }
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Xonalar ──
        ...
        header = {
            ...
            "uz": f"{flag} 1 kun / 1 kishi narxi:\n_(kattalار / 10 yoshgacha bolalar)_\n\n",
            "kz": f"{flag} 1 күн / 1 адам бағасы:\n_(ересектер / 10 жасқа дейін)_\n\n",
        }[lang]
        lines = []
        for r in rooms:
            lines.append(f"🛏 *{r['name']}* ({r['people']} чел.) — {r['adult']} / {r['child']} сум")
        text = header + "\n".join(lines)
        if len(text) > 4000:
            text = text[:4000] + "..."
        await query.edit_message_text(text, parse_mode="Markdown",
    ...
    # ── Ekskursiya ──
        ...
        text = d["excursion"][lang].format(phone=phone)
        await query.edit_message_text(text, reply_markup=back_keyboard(lang))
